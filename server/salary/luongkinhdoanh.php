 <?php
  	$query = "Select luongKinhDoanh.*, hoTen, tenCVVi,tenDangVI, maNV,thuong1,thuong2,thuong3,thuong4,chidot1,chidot2,chidot3,chidot4,chidot5
	...
	$tinhtong ="select sum(luongCB) as sumluongcb ,sum(hSCV) as sumhscv ,sum(tienhSCV) as sumtienhscv , sum(hSTN) as sumhstn ,sum(tienhSTN) as sumtienhstn ,sum(coMat) as sumcomat ,sum(nuaBuoi) as sumnuabuoi ,sum(nghiPhep) as sumnghiphep ,sum(vangMat) as sumvangmat ,sum(docHai) as sumdochai,sum(phuCap) as sumphucap,sum(ngayCom) as sumngaycom,sum(tongCom) as sumtongcom,sum(nghiPhep) as sumnghiphep,sum(tongPhep) as sumtongphep,sum(vangMat) as sumvangmat,sum(tongVang) as sumtongvang,sum(tongLuong) as sumtongLuong ,sum(sLCT) as sumsLCT,sum(sLTT) as sumsLTT,sum(tiLeSanLuong) as sumtiLeSanLuong,sum(heSoLuongTheoSanLuong) as sumheSoLuongTheoSanLuong
				from luongKinhDoanh 
				";
  ...
  ?>
                    ...
                    <thead>
                      <tr>
                        ...
                        <th rowspan="3"><p class="_w200">SL Chi Tiết</p></th>
                        <th rowspan="3"><p class="_w200">SL Thực Tế</p></th>
                        <th rowspan="3"><p class="_w200">Tỉ Lệ Sản Lượng</p></th>
                        <th rowspan="3"><p class="_w200">Hệ Số Lương Theo SL</p></th>
                 ...
                 <tbody>
                      ...
                      <tr rel="<?=$rel['adID']?>" id="<?=$rel['adID']?>">
                        ...
                        <td><?= $rel['heSoLuongTheoSanLuong'];?> VNĐ</td>
                      ...
                      <tr>
                        ...
                        <td><?= $rel ['sumsLCT']?></td>
                        <td><?= $rel ['sumsLTT']?></td>
                        <td><?= $rel ['sumtiLeSanLuong']?></td>
                        <td><?= $rel ['sumheSoLuongTheoSanLuong']?></td>
